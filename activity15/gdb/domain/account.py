...
class Account(IAccount):
    ...
    def getDailyTransferLimit(self) -> float:
        return self.get_daily_transfer_limit()

    def getRemainingDailyTransferLimit(self) -> float:
        return self.get_remaining_daily_transfer_limit()

    def canTransfer(self, amount: float) -> bool:
        return self.can_transfer(amount)

    def updateDailyTransferTotal(self, amount: float) -> None:
        self.update_daily_transfer_total(amount)

    def resetDailyTransferIfNeeded(self) -> None:
        self.reset_daily_transfer_if_needed()

    def getDailyTransferTotal(self) -> float:
        return self.get_daily_transfer_total()

    def getLastTransferDate(self) -> datetime:
        return self.get_last_transfer_date()